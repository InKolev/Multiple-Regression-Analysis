...
namespace MultipleRegression.ConsoleClient
{
    public class Program
    {
        ...
        {
            var solutionMethodFactory = new SolutionMethodFactory();
            var dataTableFormatter = new DataTableToSystemOfEquationsFormatter();
            var multipleRegressionSolver = new MultipleRegressionSolver(solutionMethodFactory, dataTableFormatter);
            ...
            var coefficients = multipleRegressionSolver.Solve(SolutionMethod.GaussianEliminationMethod, historicalData);
        }
    }
}
